...
let uiEnvironmentInstalled = false;

/**
 * 安装 UI 测试环境所需的浏览器 stub。
 *
 * 设计原因：
 * - 只有运行在 `jsdom` 的组件/页面测试才需要这些补丁
 * - 显式包成函数后，逻辑测试即使解析到这个模块，也不会真正执行任何 DOM 初始化
 */
export function installUiTestEnvironment(): void {
  if (uiEnvironmentInstalled) {
    return;
  }

  uiEnvironmentInstalled = true;
